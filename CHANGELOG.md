...
## 2026-09-19：任务「展示排序」改为位置式编号管理

- **动因**：主线任务预计达百级规模，原裸数字填写要求每次人工查询"上次填到几号"，且中途插队需要手工重排后续编号。
- **语义升级**：新增 `QuestPositionInput` 位置意图（`keep` 保持现状 / `end` 排到分类末尾 / `after` 排在某同分类任务之后 / `manual` 高级直填编号），编号由系统分配：`after` 与 `manual` 会把同分类中排位靠后的任务整体 +1 腾位（仅未删除行、排除自身、空值任务永不被动；锚点须同分类，无编号锚点视同末尾；"自身已在目标位"时跳过腾位，未变更编辑不引发重排）。腾位与写入在同一事务内完成。
- **后台交互**：`QuestForm` 的数字输入升级为"位置"下拉（编辑默认"保持现有位置（编号 n）"，新建默认"排到分类末尾"，分组列出"排在某任务之后"的参照项并带当前编号前缀，"手动指定编号"为高级折叠项）；分类下拉改为受控，切换分类时参照任务列表联动过滤并回退失效选择。任务列表页新增"排序"列，随时可查看各任务编号。
- **兼容性**：`QuestInput.sortOrder` 直填路径保持原语义（seed/verify 零改动）；`updateQuest` 未显式指定编号时由"清空"改为"保持现有值"。已新增 verify 场景 Q 断言：末尾分配、after 插入腾位、keep 不变、manual 插入腾位、跨分类参照拒绝、演练任务清理。
- 依弃用策略仅维护 SQLite 后端；Supabase 占位未变。
